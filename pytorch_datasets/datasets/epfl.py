...
class EPFL(DataSet):
    def __init__(self, root, transforms=None):
        ...
        self.dataset = []
        seq_info = open(os.path.join(root, "tripod-seq.txt"), "r").read().split('\n')
        df = pd.DataFrame({
        ...
        for seq_idx, row in df.iterrows():
            times = open(os.path.join(root, "times_{:02d}.txt".format(seq_idx + 1)), "r").read().split('\n')
            ...
            for frame in range(0, row['rotation_frames']):
                ...
                current_orientation = int(360 + current_orientation) % 360
                self.dataset.append({
                    'image_file': os.path.join(root, 'tripod_seq_{:02d}_{:03d}.jpg'.format(seq_idx + 1, frame + 1)),
                    'source': 'EPFL',
